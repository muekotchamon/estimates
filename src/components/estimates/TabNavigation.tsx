...
const tabs: Tab[] = [
  { id: 'summary', label: 'Summary', icon: <LayoutDashboardIcon className="w-4 h-4" /> },
  ...
  { id: 'production', label: 'Production', icon: <HardHatIcon className="w-4 h-4" /> },
...
export function TabNavigation({ activeTab, onTabChange, layoutVariant: layoutVariantProp }: TabNavigationProps) {
            ...
            {tabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`relative flex items-center gap-2 px-4 py-3 text-sm font-medium whitespace-nowrap transition-colors border-b-2 ${isActive ? '' : 'text-gray-500 border-transparent hover:text-gray-700 hover:border-gray-300'}`}
  ...
  // Design 3: vertical sidebar tabs (rendered inside page layout)
  return (
    <nav className="border-b border-gray-200 bg-white md:border-b-0" role="tablist" aria-label="Estimate sections">
      <div className="max-w-[1400px] mx-auto px-6 py-2">
        <div className="flex gap-1 overflow-x-auto no-scrollbar md:overflow-visible md:justify-center">
          {tabs.map((tab) => {
            const isActive = activeTab === tab.id;
            return (
              <button
